                    ...
                    <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                        ...
                        @foreach ($products as $product)                            
                            <tr>
                                ...
                                <td>{{$product->stock_amount}}</td></td>
                                <td><img src="{{asset($product->image)}}" alt="" height="50" width="80"></td></td>
                                <td>{{$product->status == 1 ? 'Published' : 'Unpulished'}}</td>
                                <td>
                                    <a href="{{route('product.detail', ['id' => $product->id]) }}" class="btn btn-info btn-sm" title="Detial">
